...
const getFileSize = (filePath: string, callback?: (err: unknown) => void): number => {
  try {
...
const getDirEntries = (dirPath: string, callback?: (err: unknown) => void): Dirent[] => {
  try {
...
const calculateTotalDirSize = (dirPath: string, callback?: (err: unknown) => void): number => {
  let totalSize = 0;
...
const getDirSizeSync = (dirPath: string, callback?: (err: unknown) => void): number => {
